...
@action(goal="Return relevant web results")
async def search_web(query: str) -> list[str]:
    await asyncio.sleep(0.01)
    return [f"Web result 1 for '{query}'", f"Web result 2 for '{query}'"]


@action(goal="Return relevant database records")
async def search_db(query: str) -> list[str]:
    await asyncio.sleep(0.01)
    return [f"DB record for '{query}'"]


@action(goal="Return cached results if available")
async def search_cache(query: str) -> list[str]:
    await asyncio.sleep(0.005)
    return []


@action(goal="Merge results from all sources into a coherent summary")
